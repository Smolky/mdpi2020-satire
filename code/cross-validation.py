...
def create_embedding_matrix (filepath, word_index, embedding_dim, encoding="utf8"):
...
def create_model (x_train, y_train, x_val, y_val, params):
    ...
    embedding_dim = params['embedding_matrix']['weights'].shape[1]
    
    
    # Get the weights
    weights = [params['embedding_matrix']['weights']]
    

    # Input layer
    main_embeddings_input = Input (shape=(None,))
    main_embeddings_1 = Embedding (vocab_size, embedding_dim, weights=weights, trainable=params['trainable'])(main_embeddings_input)
    ...
    if (params['word_embeddings_architecture'] == 'dense'):
        main_embeddings_3 = Dense (first_neuron, activation=params['activation'])(main_embeddings_2)
        main_embeddings_4 = Dropout (params['dropout'])(main_embeddings_3)
        main_embeddings_layer = Dense (second_neuron, activation=params['activation'])(main_embeddings_4)
    ...
    if (params['word_embeddings_architecture'] == 'cnn'):
        ...
        main_embeddings_4 = GlobalMaxPool1D ()(main_embeddings_3)
        main_embeddings_5 = Dropout (params['dropout'])(main_embeddings_4)
        main_embeddings_6 = Dense (second_neuron, activation=params['activation'])(main_embeddings_5)
        main_embeddings_layer = Dense (second_neuron, activation=params['activation'])(main_embeddings_6)
    
    # BiLSTM
    if (params['word_embeddings_architecture'] == 'bilstm'):
        main_embeddings_3 = Bidirectional (LSTM (first_neuron, dropout=params['dropout'], recurrent_dropout=0.2))(main_embeddings_2)
        main_embeddings_layer = Dense (second_neuron, activation=params['activation'])(main_embeddings_3)

    # GRU
    if (params['word_embeddings_architecture'] == 'gru'):
        main_embeddings_3 = Bidirectional (GRU (first_neuron, dropout=params['dropout'], return_sequences=False))(main_embeddings_2)
        main_embeddings_layer = Dense (second_neuron, activation=params['activation'])(main_embeddings_3)
...
for key, umucorpus_ids in config.ids[args.dataset].items ():
    ...
    # Split dataset
    sss = StratifiedShuffleSplit (n_splits = 1, test_size = 1 - train_ratio, random_state = seed_value)
    ...
    glove_embedding_matrix = create_embedding_matrix (pretrained_embeddings_dir + "glove.txt", tokenizer.word_index, 100)
    nonzero_elements = np.count_nonzero (np.count_nonzero (glove_embedding_matrix, axis=1))
    print ("\t...Pretrained wiki coverage: ", nonzero_elements / vocab_size)


    """
    # Spanish Unannotated Corpora
    # @link https://github.com/dccuchile/spanish-word-embeddings#fasttext-embeddings-from-spanish-wikipedia
    suc_embedding_matrix = create_embedding_matrix (pretrained_embeddings_dir + "embeddings-l-model.vec", tokenizer.word_index, 300)
    nonzero_elements = np.count_nonzero (np.count_nonzero (suc_embedding_matrix, axis=1))
    print ("\t...Pretrained suc coverage: ", nonzero_elements / vocab_size)


    # Word2Vec
    w2v_embedding_matrix = create_embedding_matrix (pretrained_embeddings_dir + "SBW-vectors-300-min5.txt", tokenizer.word_index, 300)
    ...
    # FastText
    fasttext_embedding_matrix = create_embedding_matrix (pretrained_embeddings_dir + "cc.es.300.vec", tokenizer.word_index, 300)
    ...
    p = {
        ...
        'batch_size': [64],
        'dropout': [0.5],
        'word_embeddings_architecture': ['gru'],
        'activation': ['sigmoid'],
        'embedding_matrix': [
    ...
    print (scan_object.best_model (metric='loss', asc=True))
    print ("-------------------------");
    ...
    best_model = scan_object.best_model (metric='loss', asc=True)
     
    
    # Evaluate models
    scan_object.evaluate_models (
        x_val=sentences_features_train, 
        y_val=sentences_labels_train_binary,
        n_models=1,
        metric='loss',
        folds=10,
        shuffle=True,
    ...
    with open (model_config_filename.replace ("{{ name }}", model.name), "w") as json_file:
        json_file.write (model_json)

    model.save_weights (model_weights_filename.replace ("{{ name }}", model.name))
    print ("\t...model saved")


    # Generate architecture diagram model
    plot_model (model, to_file=architecture_diagram_filename.replace ("{{ name }}", model.name), show_shapes=True, show_layer_names=True)
    ...
    plt.savefig (plot_filename.replace ("{{ name }}", model.name))
    print ("\t...plots saved")
    """
